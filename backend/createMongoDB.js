...
const createCollections = async () => {
	...
	try {
		...
		const users = db.collection( usersCollection, {
		 ...
		 const placesInfo = [
			{ Name: "favorite beach", UserID: (await (users.find({}, {projection: {Name: 0, Password: 0}}).limit(1)).toArray())[0]._id, Latitude: 62.6126, Longitude: 29.696 },
			{ Name: "favorite pizza place", UserID: (await (users.find({}, {projection: {Name: 0, Password: 0}}).skip(1).limit(1)).toArray())[0]._id, Latitude: 62.6009, Longitude: 29.7598 },
			{ Name: "favorite hiking place", UserID: (await (users.find({}, {projection: {Name: 0, Password: 0}}).skip(2).limit(1)).toArray())[0]._id, Latitude: 62.6277, Longitude: 29.8759 },
